...
export class MarketAnalysisAI {
  ...
  analyzeMarketTrends(
    ...
    const medianPrice: number = prices.length > 0 ? (prices.sort((a, b) => a - b)[Math.floor(prices.length / 2)] ?? 0) : 0;
    const averageDaysOnMarket: number = historicalData.length > 0 ? historicalData.reduce(
